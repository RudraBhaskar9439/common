...
`npm run test:all` passed 122 tests: 47 interface/integration, 16 service, 32 payment adapter, 3 controlled-browser harness, 2 storage, 1 dashboard browser journey and 21 local contract tests. TypeScript checks/build and the explicitly labeled mock demo passed. A clean `npm ci --ignore-scripts` succeeded with the committed lockfile. Local runtime: Node 26.3.1; CI uses Node 24. These are separate from network settlement evidence.
