...
export function createJobHandler(jobs: ReturnType<typeof createEvaluationJobs>, mirrorUrl: string) {
...
export async function startEvaluationService() {
  ...
  const handler = createJobHandler(jobs, process.env['HEDERA_MIRROR_NODE_URL'] ?? 'https://testnet.mirrornode.hedera.com');
