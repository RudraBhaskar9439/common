...
Set `COMMON_PUBLIC_REVIEW=yes` to let anonymous visitors open the dashboard, state, reports and report-owned screenshots/traces. Visitors get `readOnly: true` and no authenticated cookie; all evaluation and retry POSTs still require an operator session. `/login` remains available for operators. This flag requires a 24+ character operator password and does not disable the operator worker. `COMMON_READ_ONLY=yes` separately disables work for everyone.
