...
async function refresh() {
  try {
    ...
    if (state.readOnly) $('mode-note').textContent = 'Read-only review of stored evidence. New evaluations, payments, and transaction retries are disabled.';
    ...
    $('acquire').disabled = state.readOnly || busy || !state.spec; $('fresh').disabled = state.readOnly || busy || !state.spec; $('reuse').disabled = state.readOnly || busy || !selectedSpec;
